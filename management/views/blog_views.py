...
def rm(request, post_id):
    try:
        ...
        context = {'title': 'Oops!',
                   'base_tpl': 'base/full.html',
                   'text': error_text}
        return render(request, 'management/pages/error.html', RequestContext(request, context))

    if request.method == 'GET':
        context = {'title': _('Removing post #{}').format(post_id),
                   'base_tpl': 'base/full.html'}
        return render(request, 'management/pages/blog/rm.html', RequestContext(request, context))
    else:
        post.delete()
...
def index(request):
    context = {'title': _('Blog management main page'),
               'base_tpl': 'base/full.html'}
    return render(request, 'management/pages/blog/index.html', RequestContext(request, context))
